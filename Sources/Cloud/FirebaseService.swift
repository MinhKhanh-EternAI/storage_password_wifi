...
final class FirebaseService {
    ...
    // Upload toàn bộ từ store
    func syncUpload(from store: WiFiStore, completion: @escaping (Result<Void, Error>) -> Void) {
        uploadNetworks(store.items, completion: completion)
    }
    ...
    func uploadNetworks(_ networks: [WiFiNetwork], completion: @escaping (Result<Void, Error>) -> Void) {
        do {
            ...
            guard let json = String(data: data, encoding: .utf8) else {
                return completion(.failure(NSError(domain: "FirebaseService", code: -1,
                                                   userInfo: [NSLocalizedDescriptionKey: "Encode JSON failed"])))
            }
    ...
    func fetchNetworks(completion: @escaping (Result<[WiFiNetwork], Error>) -> Void) {
        docRef.getDocument { snap, error in
            ...
            guard let data = snap?.data(),
                ...
                return completion(.failure(NSError(domain: "FirebaseService", code: 404,
                                                   userInfo: [NSLocalizedDescriptionKey: "Không tìm thấy dữ liệu trên Cloud"])))
            }
